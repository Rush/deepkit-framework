...
exports.EntityState = exports.EntitySubjectStore = void 0;
const core_1 = require("@deepkit/core");
const type_1 = require("@deepkit/type");
const model_1 = require("../model");
class EntitySubjectStore {
    constructor(schema) {
        this.schema = schema;
        this.store = new Map();
    ...
    onPatch(id, version, patch) {
        ...
        if (patch.$set) {
            const $set = type_1.jsonSerializer.for(this.schema).patchDeserialize(patch.$set);
            for (const i in $set) {
                core_1.setPathValue(store.item, i, $set[i]);
            }
        }
        if (patch.$inc)
            for (const i in patch.$inc) {
                if (i === 'version')
                    continue;
                core_1.setPathValue(store.item, i, core_1.getPathValue(store.item, i) + patch.$inc[i]);
            }
        if (patch.$unset)
            for (const i in patch.$unset) {
                core_1.deletePathValue(store.item, i);
            }
        for (const fork of store.forks) {
    ...
    forkUnregistered(id, fork) {
        ...
        if (!store)
            return;
        core_1.arrayRemoveItem(store.forks, fork);
    ...
    createFork(id) {
        ...
        const fork = new model_1.EntitySubject(store.item, () => {
...
exports.EntitySubjectStore = EntitySubjectStore;
class EntityState {
    constructor() {
        this.store = new Map();
    ...
    createEntitySubject(classSchema, bodySchema, message) {
        if (message.type !== model_1.RpcTypes.ResponseEntity)
    ...
    handle(entityMessage) {
        for (const message of entityMessage.getBodies()) {
            switch (message.type) {
                case model_1.RpcTypes.EntityPatch: {
                    ...
                    const body = message.parseBody(model_1.rpcEntityPatch);
                    const store = this.getStoreByName(body.entityName);
                    store.onPatch(body.id, body.version, body.patch);
                    break;
                }
                case model_1.RpcTypes.EntityRemove: {
                    const body = message.parseBody(model_1.rpcEntityRemove);
